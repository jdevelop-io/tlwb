...
- **Pricing card.** Pro is no longer marked "coming soon": billing
  shipped with the accounts specification, so the card describes the
  plan and links to sign-in for the upgrade.
- **Hero image.** The hand-written SVG placeholder gave way to a real
  capture of the editor, `public/editor-preview.png`, regenerated by
  `scripts/capture-preview.mjs`. It doubles as the Open Graph image, so
  the tags carry absolute `https://tlwb.io` URLs.
